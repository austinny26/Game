...
@export var use_action: ItemUseAction
...
func use(user: Character)->bool:
	if use_action is ItemUseAction:
		return use_action.use(user)
	else:
		return false


# A function to instantiate the pickable item scene corresponding to this item	# Call this to drop the item back on the floor
